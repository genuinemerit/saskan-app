...
class Universe:
    ...
    def __init__(self) -> tuple:
        """
        - Load existing U, XU, GC, SS data from database.
        - Allocate structures for holding data in memory.

        Use other methods to show, add, remove or modify objects.
        """
...
class AstroIO(object):
    """Class for generating Astronomical objects.
    """
    def __init__(self):
        """Allocate class-level variables.
        """
        self.UNIV = dict()  # UniverseModel()
        self.EXTU = dict()  # ExternalUniverse()
        self.GALX = dict()
        self.STAR = dict()
        self.PLNT = dict()
        self.MOON = dict()
