...
class GameDisplay(Enum):
    ...
    """
    Don't do rendering in a data structure? Maybe
    move this to the Saskan App or a separate rendering
    class or module. Error is indicating a bad color
    argument, so maybe it is something else...
    CONSOLE_TTL_IMG =\
...
class Struct(object):
    ...
    class GameLatLong(object):
        """
        Since game lat and long refer to fantasy world locations.
    ...
    class GameLocation(object):
        ...
        based on scaling to a grid.

        Provide rough altitudes (average, min and max) meters,
        to give a general sense of the 3rd dimension. Provide
        detailed heights and depths in specialized data structures.
        """
    ...
    class Graphic(object):
        ...
        img_url: str = ''
        img_desc: str = ''
        img_type: ImageType = ImageType.JPG
...
# Pick up here testing io_data.py
# put one class at a time back in, then
# run a simple test from command line...
...
class GameGridData(object):
    ...
    data is 2D oriented, but the data-content supports 3D. Think of it
    like a layer cake or slices of scan. The rendering of the cells is
    derived from two sources: the number of rows and columns (an input
    to instantiating this class) and the placement of the grid within
    the containing map-window structure (defined in the GameDisplay class).
    To make smaller cells, assign more rows and columns to the grid; to
    make bigger cells, assign fewer rows and columns.
    """
    def _init_(self,
               p_cols: int = 1,
               p_rows: int = 1,
               ...
               p_z_down: int = 0):
        """
        The location of the grid-box within the map window is set
        in GameDisplay constants. The number of cells in the
        grid is set by init parameters for this class.
        :args:
        ...
        self.visible = False
        self.rect, self.box = self._set_grid_rects(x_offset, y_offset)
    ...
    def _set_grid_rects(self,
                        x_offset: int,
                        y_offset: int) -> tuple:
        """ Set proprietary and pygame Rect objects for the grid.
        These help place and render the grid as a whole within the
        map-window. Since the inputs are set by the constant structure
        'GameDisplay', this method is treated as "internal". There is
        no attempt to provide skew and so on for 3D rendering. This
        assumes a 2D rendering of each "z-layer" of the grid, with
        default being the z-zero layer.
        :returns:
        ...
        return (grid_rect, grid_rect_pygame)
    ...
    def set_grid_size(self,
        ...
        This determines how many cells are contained in the grid/matrix.
        In this case, a 3D matrix is supported. The z-dimension is handled
        separately for "up" and "down" rather than using positive and
        negative values.
        :returns:
        - (rc, zz): tuple of column/row and up/down dimensions
        """
    ...
    def set_cell_size(self,
        ...
        z-zero plane, that is, "height" in the sense of a 2D grid,
        not in the sense of the z-directions of the 3D grid.
        :returns:
        {w, h}: width, height of each cell
        """
        w = int(round(GameDisplay.GAMEMAP_W - x_offset) / self.grid_size.c)
        h = int(round(GameDisplay.GAMEMAP_H - y_offset) / self.grid_size.r)
        return {'w': w, 'h': h}
    ...
    def set_grid_lines(self,
        ...
        w = self.cell_size['w'] * self.grid_size.c
        h = self.cell_size['h'] * self.grid_size.r
    ...
    def set_grid_data(self) -> dict:
        ...
        including the z directions, which are keyed as positive (up)
        and negative (down).
        """
        grid_matrix = dict()
        cell_data = {
        ...
        for r in range(self.grid_size.r + 1):
            grid_matrix[r] = dict()
            for c in range(self.grid_size.c + 1):
                grid_matrix[r][c] = dict()
                for z in range(self.grid_size.z_up + 1):
                    grid_matrix[r][c][z] = dict()
                    grid_matrix[r][c][z] = cell_data
                for d in range(self.grid_size.z_down + 1):
                    z = d * -1
                    grid_matrix[r][c][z] = dict()
                    grid_matrix[r][c][z] = cell_data
        return grid_matrix
...
class GalacticCluster(object):
    ...
    univ_nm_fk: str = ''
    center_from_univ_center_gly: Struct.CoordXYZ
...
class Galaxy(object):
    ...
    center_from_univ_center_kpc: Struct.CoordXYZ
    ...
    */
    ...
    def constraints(cls):
        return {
            ...
            "GROUP": {"map_loc": Struct.GameLocation},
            "ORDER": ["map_nm_pk ASC"]
        }
...
class CharMember(object):
    ...
    def constraints(cls):
        return {
            ...
            "GROUP": {"char_member": Struct.Graphic},
...
# The methods below may be better suited in io_db...
...
class GetGameData(object):
    ...
    # All of the following needs to be reviewed, refactored
    # to use the improved classes, methods above and to use
    # DB-based data where appropriate.
    # ==================================
    ...
    def compute_map_scale(self,
        ...
        if map['ln']['km']['w'] > GameDisplay.G_LNS_KM_W:
            err = f"Map km w {map['w']} > grid km w {GameDisplay.G_LNS_KM_W}"
        if map['ln']['km']['h'] > GameDisplay.G_LNS_KM_H:
            err = f"Map km h {map['h']} > grid km h {GameDisplay.G_LNS_KM_H}"
        if err != "":
        ...
        map['ln']['ratio'] =\
            {'w': round((map['ln']['km']['w'] / GameDisplay.G_LNS_KM_W), 4),
             'h': round((map['ln']['km']['h'] / GameDisplay.G_LNS_KM_H), 4)}
        # Compute map line dimensions in px
        # Multiply grid line px w, h by map ratio w, h
        map['ln']['px'] =\
            {'w': int(round(GameDisplay.G_LNS_PX_W * map['ln']['ratio']['w'])),
             'h': int(round(GameDisplay.G_LNS_PX_H * map['ln']['ratio']['h']))}
        ...
        map['ln']['px']['left'] =\
            int(round((GameDisplay.G_LNS_PX_W - map['ln']['px']['w']) / 2) +
                GameDisplay.GRID_OFFSET_X)
        map['ln']['px']['top'] =\
            int(round((GameDisplay.G_LNS_PX_H - map['ln']['px']['h']) / 2) +
                (GameDisplay.GRID_OFFSET_Y * 4))
    ...
    def set_gamemap_dims(self,
        ...
        displays
           over the "grid" on the GAMEMAP GameDisplay.
        :attr:
        ...
        self.G_CELLS["map"]["s_rect"] =\
            CompareRect.set_rect(map_px["top"], map_px["left"],
                                 map_px["w"], map_px["h"])
