...
class MapMsg():
    """ If string-based (JSON) messages, deserialize to dict format.
        ...
        Execute the service.
        This class mimics an API service controller.
        Eventually we'd want to make this work asynchronously, supporting parallel calls.
    """
    msg = None
    reg = None
    pub = None
    ...
    def clear(self):
        """ Wipe the message """
        self.msg = {
            "type": BC.STR_Ø, "id": BC.STR_Ø, "size_xy": BC.TUPINT2_Ø, "desc": BC.STR_Ø,
            "name": BC.STR_Ø, "title": BC.STR_Ø, "style": BC.STR_Ø,
            "path": BC.STR_Ø, "itmtyp": BC.STR_Ø,
            "gid": BC.STR_Ø, "draw": BC.STR_Ø, "fill": BC.STR_Ø, "stroke": BC.STR_Ø,
            "zix": BC.INT_Ø,
            "tabix":  BC.INT_Ø, "thick": BC.NUM_Ø, "opaq": BC.NUM_Ø, "dash": BC.TUPINT2_Ø,
            "move": BC.TUPFLT2_Ø, "fontsize": BC.INT_Ø, "fontfam": BC.STR_Ø,
            "loc_xy": BC.TUPINT2_Ø, "words": BC.STR_Ø, "uid": BC.STR_Ø, "img_id": BC.STR_Ø,
            "delete": bool}
        self.reg = {
            'gen': ["type", "id", "desc"],
    ...
    def make(self, p_msg, maker):
        ...
        def __check_fmt(v_msg):
            ...
            if not isinstance(v_msg, dict):
                FC.log_me('Unknown message format: <{0}>'.format(
                    str(type(p_msg))), BC.ERROR)
                self.pub = False
            elif 'type' not in v_msg.keys():
                FC.log_me('Missing required message key:  <type:>', BC.ERROR)
                self.pub = False
            elif v_msg['type'] not in self.reg:
                FC.log_me('Unknown message type: <{0}>'.format(
                    v_msg['type']), BC.ERROR)
                self.pub = False
            return self.pub
        ...
        def __set_msg(v_msg):
            ...
            for attr in v_msg:
                if attr not in self.msg:
                    FC.log_me(
                        'Attribute not recognized: <{0}>'.format(attr), BC.WARN)
            ...
            for attr in v_msg:
                if attr not in pub_attrs:
                    FC.log_me('Attribute not used: <{0}>'.format(attr), BC.INFO)
    ...
    def serve(self, maker):
        ...
        if self.msg['type'] == 'svg':
            ...
            FC.log_me('No service found for: <{0}>'.format(
                self.msg['type']), BC.WARN)
...
class MakeMap():
    ...
    def __set_string(cls, p_value, p_escape=False):
        ...
        """
        v_value = None
        if p_value not in [0, '', None, BC.STR_Ø]:
            if p_escape:
    ...
    def __set_number(cls, p_value, p_float=False):
        ...
        """
        v_value = None
        if p_value in [None, '', BC.NUM_Ø, BC.INT_Ø]:
            v_value = None
    ...
    def __set_num_pair(self, p_value, p_float=False):
        ...
        """
        v_value = None
        if p_value in [None, 0, '', BC.TUPINT2_Ø, BC.TUPFLT2_Ø]:
            v_value = None
    ...
    def __set_id(self):
        ...
        """
        if 'id' not in self.msg or self.msg.id in [None, 0, "", BC.STR_Ø]:
        ...
        for k_type in self.catalog:
            if self.msg.id in self.catalog[k_type]:
                FC.log_me('ID already in catalog: <{0}>'.format(self.msg.id), BC.WARN)
        if self.msg.id in self.file_ls:
            FC.log_me('ID already in file list: <{0}>'.format(self.msg.id), BC.WARN)

    def __set_name(self):
        """ Set element Name value. If no name provided, set Name to ID. """
        if 'name' not in self.msg or self.msg.name in [None, 0, "", BC.STR_Ø]:
    ...
    def __set_indent(self):
        ...
        tlvl = BC.T1 * self.msg.level
        self.ntab = {}
        self.ntab[0] = BC.NT0 + tlvl
        self.ntab[1] = BC.NT1 + tlvl
    ...
    def __set_comment(self):
        ...
        comment = ''
        if 'desc' in self.msg and self.msg.desc not in [None, '', BC.STR_Ø]:
    ...
    def __set_basic_attrs(self, p_msg):
        ...
        def_level = {'svg': 0, 'ref': 1, 'grp': 1, 'tgp': 2,
    ...
    def __set_size_xy(self):
        ...
        if 'size_xy' not in self.msg or\
                self.msg.size_xy in [None, '', (), BC.TUPINT2_Ø]:
            self.msg['size_xy'] = (400, 400)
            FC.log_me('Set to default (400, 400): <size_xy>', BC.INFO)
        ...
        if self.msg.size_xy[0] in [None, '']:
            size_xy = (400, self.msg.size_xy[1])
            self.msg.size_xy = size_xy
            FC.log_me('Set to default (400, _): <size_xy[0]..x:width>', BC.INFO)
        if self.msg.size_xy[1] in [None, '']:
            size_xy = (self.msg.size_xy[0], 400)
            self.msg.size_xy = size_xy
            FC.log_me('Set to default (_, 400): <size_xy[1]..y:height>', BC.INFO)
    ...
    def __set_file(self, path_nm=None):
        """ Set name and path of a static file and add it to the file_ls.
            ...
            If the file is already on the file list, then modify the message ID to match it.
        """
        if path_nm is None:
            path_nm = 'static/' + self.msg.name + '.' + self.msg.type
        found_it = False
        for file_key, item in self.file_ls.items():
            if item.path.rqst == path_nm:
                FC.log_me('File name already registered: <{0}>'.format(path_nm), BC.WARN)
                found_it = True
                self.msg.id = file_key
        if not found_it:
            self.file_ls[self.msg.id] = DotDict({"type": str, "path": None})
            self.file_ls[self.msg.id].type = self.msg.type
            self.file_ls[self.msg.id].path = FC.get_path(path_nm)
    ...
    def __set_path(self):
        ...
        """
        if 'path' not in self.msg or self.msg.path in [None, '', BC.STR_Ø]:
            FC.log_me(
                'Valid file path is required when defining a <ref> message', BC.ERROR)
        path_nm = str(self.msg.path)
        v_path = FC.get_path(path_nm)
        if not v_path.exists:
            FC.log_me('Invalid reference. File not found: <{0}>'.format(
                path_nm), BC.ERROR)
    ...
    def __set_itmtyp(self):
        ...
        if 'itmtyp' not in self.msg \
                or self.msg.itmtyp in [None, '', BC.STR_Ø] \
                or self.msg.itmtyp not in ['script', 'image']:
            FC.log_me('Missing or invalid reference item type: <{0}>'.format(str(self.msg.itmtyp)),
                      BC.ERROR)
    ...
    def __set_gid(self):
        ...
        """
        if self.msg.gid in [None, 0, '', BC.STR_Ø]:
            self.msg.gid = None
        else:
            ...
            if not parent_found:
                FC.log_me('Parent group not found: <{0}>'.format(
                    self.msg.gid), BC.ERROR)

    def __set_uid(self):
        """ Set Use ID. Reference another item which is already defined internally. """
        if self.msg.uid in [None, 0, '', BC.STR_Ø]:
            self.msg.uid = None
        else:
            use_found = False
            for k_type in self.catalog:
    ...
    def __render_desc(self):
        ...
        if self.msg.desc is not None:
            self.code += "{0}<desc>{1}</desc>".format(
                self.ntab[1], self.msg.desc)
    ...
    def __asm_kids(self, pkey: str):
        ...
        for msgt in ['grp', 'tgp', 'elm', 'txt', 'img', 'use']:
            ...
            #            for cat_key, cat_val in self.catalog[msgt].items()
            #            if msgt in self.catalog and cat_val['msg'].gid == pkey]
            if msgt in self.catalog:
                for cat_key, cat_val in self.catalog[msgt].items():

                    # pp(("cat_key: ", cat_key, "cat_val: ", cat_val))

                    if cat_val['msg'].gid == pkey:
                        kordkeys.append((cat_val['msg'].order, cat_key))

            # Sort by hierarchical order:
            # pp(("pkey:", pkey, "msgt: ", msgt, "kordkeys: ", kordkeys))
    ...
    def make_svg(self, p_msg: dict):
        ...
        self.code += '{0}<svg xmlns="http://www.w3.org/2000/svg"'.format(
            self.ntab[0])
        ...
        if self.msg.title is not None:
            self.code += "{0}<title>{1}</title>".format(
                self.ntab[1], self.msg.title)
        self.__render_desc()
        self.__catalog_item()
    ...
    def make_ref(self, p_msg):
        ...
        if self.msg.itmtyp == 'image':
            self.code += "{0}<!-- Reference image: {1} -->".format(
                self.ntab[0], self.msg.path)
        else:
            self.code += '{0}<script type="text/ecmascript"'.format(
                self.ntab[0])
            self.code += ' xlink:href="{0}"'.format(
                self.file_ls[self.msg.id].path.abs)
    ...
    def make_grp(self, p_msg):
        ...
        if self.msg.opaq is not None:
            self.code += ' stroke-opacity="{0}"'.format(self.msg.opaq)
    ...
    def make_txt(self, p_msg):
        ...
        if self.msg.loc_xy is not None:
            self.code += ' x="{0}" y="{1}"'.format(
                self.msg.loc_xy[0], self.msg.loc_xy[1])
        self.__render_zix()
        self.__render_tabix()
    ...
    def make_img(self, p_msg):
        ...
        self.code += ' xlink:href="{0}"'.format(
            self.file_ls[self.msg.img_id]['path'].abs)
        if self.msg.loc_xy is not None:
            self.code += ' x="{0}" y="{1}"'.format(
                self.msg.loc_xy[0], self.msg.loc_xy[1])
    ...
    def make_axs(self):
        ...
        for ⅎuid, ⅎmax in [('x_mark', maxX), ('y_mark', maxY)]:
            ⅎmark = ⅎinc
            while ⅎmark < ⅎmax:
                ⅎXY = {'x_mark': ((ⅎmark, 0), 0),
                       'y_mark': ((0, ⅎmark), 10000)}
...
if __name__ == "__main__":
    ...
    MS.make({'type': 'txt', 'gid': 'compass', 'loc_xy': (
        30, 30), 'words': "NW", "zix": 2}, MM)
    MS.make({'type': 'txt', 'gid': 'compass', 'loc_xy': (
        30, 1790), 'words': "SW", "zix": 2}, MM)
    MS.make({'type': 'txt', 'gid': 'compass', 'loc_xy': (
        2050, 1790), 'words': "SE", "zix": 2}, MM)
    MS.make({'type': 'txt', 'gid': 'compass', 'loc_xy': (
        2050, 30), 'words': "NE", "zix": 2}, MM)
    # IMG message
