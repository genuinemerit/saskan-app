...
# -*- coding: utf-8 -*-
"""
:module:    bow_creds_model

REST services for handling physical IO for credentials-related tasks.
Back end API services for Data Management
...
Should be called only by bow_data APIs.
...
class CredsConstants(object):
    ...
    def __init__(self):
        ...
        self.setup_request_body =  {
            ...
            "cred_req":  "list of 1 to n pari-groups with 'method', 'app', 'path' keys" }
        ...
        self.v_0_1_meta_paths = {'v0.1': [
            [["GET"], ["/admin/creds/model/info"],   "Describe how to handle authorization set-up requests"],
            [["GET"], ["/admin/creds/model/get"],        "Describe how to retrieve info about an authorization set-up"],
            [["GET"], ["/admin/creds/model/delete"],     "Describe how to remove an authorization set-up"],
            [["GET"], ["/admin/creds/model/put"],        "Describe how to create an authorization set-up"] ] }

        self.v_0_1_active_paths = {'v0.1': [
            [["GET", "DELETE"], ["/admin/creds/model/"], ["retrieval request package"], "Retrieve or delete credentials"],
            [["PUT"],           ["/admin/creds/model/"], ["setup request package"], "Add credentials"] ] }
...
class CredsMeta(web.RequestHandler):
    ...
    def get(self, *args, **kwargs):
        ...
        """
        meta_topic = self.request.uri.split('/')[4]
        pp(self.request.uri)
        ...
        if meta_topic == 'info':
            ...
            meta_msg['request']['paths'] = ["/admin/creds/model/"]
            if meta_topic == 'get':
...
class Creds(web.RequestHandler):
    ...
    @classmethod
    def load_creds_workbook(cls):
        """
        Load the bow_data_creds workbook into memory as a series of pandas dataframes
        """
        data_file = os.path.join(os.path.dirname(__file__),
                                    '..',
                                    options.data_secrets_path,
                                    options.data_secrets_book)
        xlpath = FB.get_path(data_file)
        logging.debug('xlpath : {}'.format(xlpath.abs))
        if not xlpath.exists or not xlpath.isFile:
            logging.error("{} does not exist".format(data_file))
            raise ValueError
        with warnings.catch_warnings():
            warnings.filterwarnings("ignore", category=PendingDeprecationWarning)
            warnings.filterwarnings("ignore", category=DeprecationWarning)
            xlfile = pd.ExcelFile(xlpath.abs)
        creds_df = dict()
        for sheet_nm in xlfile.sheet_names:
            creds_df[sheet_nm] = xlfile.parse(sheet_nm)
        return creds_df
    ...
    def put(self, *args, **kwargs):
        ...
        def write_creds():
            """
            Update resource located in bow_data/store
            In this case, at:

            ~/bow/data/bow_data/store/vault/.secrets

            For now, I'll store the data in CSV-structured files that I can export easily to spreadsheets.
            All rows are versioned and time stamped.

            __user.csv__ defines (for now) app-names who are acceptable

               Later we'll expand on that for various kinds of end-users
               The encryption-key for a given user is stored in this file
               It is created the first time we create a permission key for a user
               Later we can create services specifically to refresh a user's encryption key
               Not using the encryption key yet, but will later.

               A permission key is also generated and stored on this file.
               This is what a user passes in to be authorized to do stuff.
               The "creds" setup activity adds this value.
               My thinking is that this key would be refreshed regularly, based on timeout.

            __action.csv__ defines what method / path combinations are supported by the system
                Permissions can be granted at the action level

            __role.csv__ groups similar sets of actions by a name
                This file just names/describes the role
                A user with a role-level authorization is permitted to use the actions in that role

            __role_action.csv__
                Lists all role + action combinations

            __role_permission.csv__
                Lists all user + role entitlements

            __action_permission.csv__
                List all user + action entitlements

            These are actually stored as a LibreOffice spreadsheet named bow_data_creds.ods
            There's a xlsx version of it too. Each of the files noted above is a tab.
            """

            return (400, 'This is a test')
        ...
        creds_df = self.load_creds_workbook()
        logging.debug(creds_df)
        status_code, alert = write_creds()
...
def main():
    ...
    app = web.Application(
        handlers=[
            (r"{}/creds/model/info".format(options.app_url_root), CredsMeta),
            (r"{}/creds/model/get".format(options.app_url_root), CredsMeta),
            (r"{}/creds/model/delete".format(options.app_url_root), CredsMeta),
            (r"{}/creds/model/put".format(options.app_url_root), CredsMeta),

            (r"{}/creds/model".format(options.app_url_root), Creds),
...
if __name__ == "__main__":
    ...
    define_options = ['host_domain', 'app_cname', 'app_ports', 'app_url_root',
                      'data_secrets_path', 'data_secrets_book']
