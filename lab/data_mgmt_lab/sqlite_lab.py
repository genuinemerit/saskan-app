...
Do database creation and data management.

- Schema file --> generate and run DDL and DML code.
- Update records --> logical delete + insert.
- Remove records --> only when a purge is run.

Launched by
    - Controls --> passes it to Editor
...
- ts_init = When oid (logical item) and uid (physical row) added
- ts_updt = When uid (physical row) first added or last updated
- ts_remv = When uid (physical row) logically deleted
- hash_id =   Hash all of the non-audit values in a physical row

Encryption is not managed at either the row or column level.
If interested in encryption, see docs on sqlite3 DB management and/or
encryption of the underlying storage media.
"""
import json
...
class DBase(object):
    """Support database setup, usage, maintenance."""

    def __init__(self, logger: object, schema_path: str):
        ...
        """
        self.APP = path.join(UT.get_home(), TX.db.app_dir)
        self.LOG = logger
        self.LOG.write_log(ST.LogLevel.DEBUG,
                           TX.ms.init_trace + ": {}".format(__name__))
        with open(schema_path, "r") as pf:
            self.SCH = json.loads(pf.read())
        pf.close()
        self.dbact = ('add', 'upd', 'del')
    ...
    def disconnect_dmain(self):
        """Drop connection to main db at specified path."""
    ...
    def connect_dmain(self):
        """Open connection to main data base.
        ...
        self.dmain_conn = sq3.connect(main_db)
        self.LOG.write_log(ST.LogLevel.DEBUG,
                           TX.ms.db_connect + ": {}".format(main_db))
